...
2. 维护者信息
3. 定义环境变量
    1. **注意低层的环境变量会被带到上层**(?)，比如golang官方镜像的`GOPROXY=https://proxy.golang.org,direct`，但是我们一般设置为`GOPROXY=https://goproxy.cn,direct`，所以需要重新设置
...
如何在控制台打印输出：默认情况下Dockerfile中的`RUN echo xxx`是不会将xxx打印到控制台的，可以加上参数来让它打印`docker build ... --progress=plain --no-cache`
