...
# 三 基础
## 0 架构和常见词语
###  扇区对齐(4K对齐)、簇对齐和内存对齐、伪共享
概述：簇对齐和扇区对齐(4K对齐)是对于硬盘数据的概念，内存对齐是内存里数据的概念。
1. 扇区对齐(4K对齐)：硬盘分区起点对齐硬盘物理扇区。读取硬盘数据时是按扇区读取的，4k没对齐的话，分配的簇也会横跨两个扇区，读取1簇必然要跨2个物理4k扇区，导致两次IO；4k对齐的话，1簇刚好落在1个完整物理4K扇区内，读取1簇一次IO就能搞定。
    1. 物理扇区(Sector)和逻辑扇区：扇区是硬盘物理层面最小读写单元。老式机械盘的物理扇区是512B，现在新盘的物理扇区是4k。为了兼容老盘，将4k划分为8个逻辑扇区。所以逻辑扇区和物理扇区永远对齐，与系统无关。
    1. 分区：将物理硬盘人为地分为一块或多块逻辑分区。
    3. 簇(Cluster)和块(Block)：文件系统在逻辑层面给文件分配空间的最小单位。
        1. Windows叫Cluster，Linux叫Block。
        1. 簇的大小：常见是4k，也可以设置成其他大小。
        1. 1个文件最少占1个簇：哪怕文件大小只有1字节。
        2. 文件真实大小 ≠ 占用的空间
2. 簇对齐：簇和逻辑扇区对齐。系统将硬盘格式化后，永远都是簇对齐的。
3. 内存对齐（Memory Alignment）：CPU 读取内存时，不是一个字节一个字节读，而是按固定块大小（如 4 字节、8 字节）读取。为了让 CPU 高效读写数据，编译器会自动给变量安排内存位置、插入填充字节，这个过程就叫内存对齐。
    1. 内存对齐的目的
        1. 提高CPU读取效率：CPU读取变量更快
        2. 兼容硬件：有些硬件不支持读取未对齐的内存
    2. 内存对齐的规则
        1. 对齐数：即对齐值，它决定了变量在内存中起始地址必须是多少的整数倍。
            1. 基本数据类型对齐数等于该类型本身的大小：比如 char=1，int=4，double=8
            2. 结构体对齐数 = 成员中最大的基本类型大小。嵌套结构体同理。
            3. ：被修改的
        2. 对齐地址：每个变量的起始地址必须是自身对齐数的整数倍，不满足时编译器会自动插入填充字节(padding)。填充字节只是为了占位，所在的块会被读取，但里面的内容不会被读取。
        3. 结构体里成员变量的最优顺序：从小到大或从大到小排列都是最优。这种布局更紧凑，更节约空间。
        4. padding是编译时确定还是运行时确定：编译时就确定。用`Sizeof()`、`Offsetof()`、`Alignof()`这类编译期常量可以查看。
    3. 深入了解
        1. 数据的流动顺序：硬盘 (ROM/SSD) → RAM主存 → CPU Cache (L1/L2/L3) → CPU通用寄存器
            1. RAM主存：即内存条，我们通常说的内存就是它，程序中变量实例一般都生成在这儿。CPU缺数据时，从RAM主存一次性搬运一个CacheLine(现代CPU缓存行标准大小为64B)进缓存
            2. CPU Cache (L1/L2/L3)：集成在CPU内部，速度远快于RAM主存、但容量很小。程序数据优先放 Cache；命中就从 Cache 拿，没命中就去 RAM 拖 64B
            3. CPU通用寄存器：4B/8B存取，速度最快但极小。**内存对齐的4/8B对齐，就是针对Cache→寄存器的访问规则。**
4. 伪共享(False Sharing)：略
    1. 多核缓存同步MESI协议：略

### 字符集和字符编码
概述：字符集是「字的集合」，规定有哪些字符、每个字符对应一个唯一编号（码点 / 码值），只做「编号分配」，不涉及二进制存储。字符编码是「把字的编号转成二进制数字的规则」，负责落地存储、传输。
...
3. 繁体字
    1. Big5字符集和字符编码
4. 西欧拉丁
    1. ISO-8859-1/Latin1/Latin-1
5. Unicode字符集
    1. UCS-2字符编码：略
    2. UTF字符编码

ASCII码：范围是0~127，即`\x00`-`\x7f`，分为控制字符（不可打印）和可打印字符
1. 控制字符（不可打印）和可打印字符
    1. 控制字符（不可打印）：ASCII编码表中有33个控制字符串(0–31和127)，用于控制像打印机电报机等一些外围设备。例如，12代表换页/新页功能，此命令指示打印机跳到下一页的开头；127表示delete。
        1. 控制字符（除了水平制表符、换行、回车之外）在 HTML 文档中不起任何作用。
    2. 打印字符：数字 32–126 分配给了可打印字符。也称为可显示字符。
2. 扩展ASCII(Extended ASCII)：Extended ASCII没有官方统一标准，都是各自实现，一般不推荐使用。常见的实现有
    1. DOS时代的CP437：IBM当年为PC扩展的8位码，范围是128~255，即`\x80`-`\xff`,它扩展的是一些打印字符
    2. Windows早期的CP1252：西欧扩展
3. 表示方法：最早是C语言规范的，后续的其他语言基本都是延续了C语言的规范。分为数值和字符两种方式。
    1. 数值表示：打印出来的是ASCII码的数值而不是字符。
        1. 二进制
        2. 八进制
        3. 十六进制
    2. 字符表示：打印出来的是ASCII字符而不是数值。
        1. 二进制
        1. 八进制： 以`\`加上3个八进制数字来表示，其中的第一个数字不能大于3。

            ```go
            var num = "\060" // 打印 0
            var num = "\101" // 打印 A
            ```
        1. 十六进制：`\x`加上字符对应的两个十六进制加上双引号来表示。它的最大范围是`\x00`到`\xff`,其中`\x00`-`\x7f`用来表示标准ASCII字符，`\x80`-`\xff`用来表示扩展ASCII字符。比如`\x4e`表示字母N。

ISO-8859-1/Latin1/Latin-1：ISO-8859-1 标准名，日常俗称 Latin1 / Latin-1。ISO-8859-1是单字节编码，编码范围**使用了单字节内的所有空间**(将一个字节的8位都用起来了)，在支持ISO-8859-1的系统中传输和存储其他任何编码的字节流都不会被抛弃。换言之，把其他任何编码的字节流当作ISO-8859-1编码看待都没有问题。MySQL数据库默认编码是Latin1就是利用了这个特性。ASCII编码是一个7位的容器，ISO-8859-1编码是一个8位的容器。所以它向下兼容ASCII，其编码范围是0x00-0xFF，0x00-0x7F之间完全和ASCII一致，0x80-0x9F之间是控制字符，0xA0-0xFF之间是文字符号。主要用于西欧拉丁字符，无法表示中文。
1. 使用场景
    1. 早期HTTP协议默认编码、Java早期`byte[]`和`String`互转默认用 ISO-8859-1、MySQL老版本默认编码
    1. 二进制兜底传输：iso8859-1是单字节编码，当需要把原始二进制字节流（图片、文件、加密数据、混合字节），临时当成「字符串」在接口、协议、内存里传递时，可以用它。不过更推荐用Base64，因为Latin1包含回车、换行等不可见字符，传进JSON、HTTP、XML等协议里会乱码或截断，而Base64则没这个问题。

GB2312、GBK和GB18030：汉字的国标码，专门用来表示汉字，是双字节编码，而英文字母和iso8859-1一致（兼容iso8859-1编码）。其中GBK编码能够用来同时表示繁体字和简体字，而gb2312只能表示简体字，GBK是兼容gb2312编码的。
1. GB2312：1980年推出，变长双字节编码，汉字和全角2字节，其他1字节。
2. GBK：1995年推出，变长双字节编码
3. GB18030：2000年推出，现行是2022年标准.
    1. 字符编码：变长四字节(一二四)编码。一字节等同于ASCII码，二字节对应GBK，四字节覆盖Unicode全部剩余码位。

Unicode：它是一个字符集、符号集，Unicode给每一个字符分配一个唯一编号，用编号来表示所有语言的字符。在很多软件内部是使用Unicode编码来处理的，比如java、go。Unicode也称为统一码、万国码、单一码。Unicode只规定了符号的编号，却没有规定这个编号应该如何存储。
1. 参考
    1. [https://home.unicode.org/](https://home.unicode.org/)
1. 概述
    1. 空间、平面（Plane）和Code Point：Unicode把所有字符，放进一个超大编号空间，范围`U+0000`至`U+10FFFF`。整个空间被分成 17 个平面（Plane），每个平面有 65536 个码位
        1. BMP：17个平面里最核心、最常用的平面是第0号平面Plane 0，也就是BMP(Basic Multilingual Plane，基本多文种平面)，日常使用到的绝大多数字符都在BMP里。
        2. Code Point：Unicode给每一个字符分配一个唯一的纯数值编号，范围`0 ~ 0x10FFFF`，这个编号就是Code Point。但为了易读性和表明这个数值是Unicode Code Point，通常转为十六进制，并带上前缀`U+`，格式形如`U+xxxx`，比如汉字“中”的Code Point是`U+4E2D`。其他字符编码对接Unicode，处理的就是这个Code Point数值。Code Point也称为码位、码点。
            1. 表示方法
                1. 方法一：以反斜杠和Unicode的首字母开头加上字符`\uxxxx`或`\Uxxxx`：其中`xxxx`是十六进制。比如
                    1. "严"的Unicode编码是`\u4e25`或`\u4E25`。
                    2. `\u0000`到`\u007F`对应全部128个ACSII字符
                2. 方法二：形如`U+xxxx`:比如`U+4E2D`表示"中"
            2. 字符长度：Unicode仅定义字符与Code Point的一一对应关系，不规定存储长度，所以没有字符长度的概念。Code Point的范围是`U+0000`至`U+10FFFF`，单个Code Point数值大小不固定。
...
UTF(Unicode Transformation Format)：为了解决Unicode编号如何存储，产生了UTF编码，UTF编码是Unicode的一种实现方式。UTF编码可以用来表示所有语言的字符，兼容iso8859-1编码。UTF编码自带简单的校验功能。
1. 分类：主流分为UTF-8、UTF-16和UTF-32
    1. UTF-8：变长编码
        1. 字符长度：每个字符的理论长度是1至6字节，实际只用1至4字节，因为4字节足够覆盖全世界所有字符，5、6字节只是协议预留。一般英文字母1个字节、希腊阿拉伯2字节、汉字使用3或4个字节(绝大多数3字节，少数4字节)。
        2. UTF-8 中 0–127 与 ASCII 完全相同
    2. UTF-16：变长编码。兼容UCS-2
        1. 字符长度：2字节或4字节。常用汉字固定2字节，生僻字和表情4字节（2 个 UTF-16 单元，即代理对（surrogate pair））。
    3. UTF-32：定长编码
        1. 字符长度：固定4字节
2. 经验
    1. 虽然UTF是为了使用更少的空间而使用的，但那只是相对于Unicode编码来说，如果已经知道是汉字，则使用汉字的国标码无疑是最节省的。
    2. 虽然UTF编码对汉字使用3个字节，但即使对于汉字网页，UTF编码也会比Unicode编码节省，因为网页中包含了很多的英文字符。

问题
1. ANSI是什么，和ASCII码什么关系：ANSI有两个含义，一是指美国国家标准学会(American National Standards Institute)，是一个标准化机构，它制定了ASCII码；二是在Windows系统里，ANSI也指“本地默认编码” ，用以表示当前Windows区域设置对应的默认多字节编码，比如简体中文Windows的ANSI是GB2312，繁体中文Windows的ANSI是Big5等等，这些ANSI编码的共同特点是向下兼容ASCII码，不兼容UTF-8
    1. 记事本 “另存为” 编码的选项ANSI就是含义二：当前系统默认的多字节编码（比如中文 = GBK）

### 常见词语
#### 完整限定名（Fully Qualified Name）
用于唯一标识某个实体的名称，通常包含命名空间（或包）、类型（或类）等信息
...
1. 通俗易懂的解释：https://objccn.io/issue-13-1/
2. MVVM核心的思想就是视图是状态的函数：View = ViewModel(Model)，所以当Model发生改变时，ViewModel会来操作View来怎么做，而非是自己写代码来做。Model 层代表数据模型，也可以在Model中定义数据修改和操作的业务逻辑；View 代表UI 组件，它负责将数据模型转化成UI 展现出来，ViewModel 是一个同步View 和 Model的对象。

#### new,工厂模式和依赖注入的通俗易懂比较
参考：
...
1. 参考
    1. 维基百科:[https://en.wikipedia.org/wiki/Method_stub](https://en.wikipedia.org/wiki/Method_stub)
...
ANSIC进一步对IO的概念进行抽象，在C语言中，流是一个流动的数据缓冲区，数据从数据源方向经过缓冲区流向数据的目的地。所有的IO操作只是简单地从程序移进或者移出字节，这种流动的字节便被称为流（stream）。程序员只需要关心创建正确的输出字节数据，以及正确的解释从输入读取的字节数据，特定IO设备的细节对程序员是隐藏的。
...
在LINUX系统中，文件分为两种：文本文件和二进制文件。在C语言中流分为两种类型：文本流（text stream）和二进制流（binary stream）。其他语言中的字符流和字节流亦是同理。
...
二进制流中的数据是按照二进制编码的方式来存放文件的。比如数十进制的5678的二进制为1011000101110，大于8位不足16位，补足16位变成00010110 00101110，只占二个字节。二进制数据也可在屏幕上显示， 但其内容无法读懂。
