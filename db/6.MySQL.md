...
```sql
-- 查看第n条数据
select name from products  where ... LIMIT 1;
...
## 1 mysql命令行
使用：
1. 登录：`mysql -u userNameA -p passwordA -h localhost -P port_num`
...
10. 执行SQL文件：有两种方法
    ...
    2. 进入到mysql控制台，执行`source absolute_path/xxx.sql`或者`\. absolute_path/xxx.sql`
        ...
        ```bash
        # windows
        source C:/Users/xxx/projectA/migrations/sql_create_xxx.sql; # 注意不是用反斜杠
        ```
