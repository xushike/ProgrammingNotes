...
1. 块级元素
    ...
    `p`, `ul`, `form`, `div`等元素被称为块级元素
2. 行内元素

    `span`, `input`等元素称为行内元素
3. 块级元素和行内元素比较

    主要区别就是块级元素会从上到下一个个垂直排列,每个自占一行,而行内元素在一行中水平排列,行内元素的高度由其内容撑开,不可显示的设置其高度,这就是为什么我们一次次的在span上设置height属性不好使的原因.
    1. 那么如何设置块级元素水平排列?
...
### 2 锚点(anchor)
网页制作中超级链接的一种，又叫命名锚记。命名锚记像一个迅速定位器一样是一种页面内的超级链接，运用相当普遍。
