...
常用对象有:console,crypto,document,history,location
...
3. 第一个子节点,前一个节点,后一个节点,最后一个子节点
    ...
    2. `previousSibling`
    2. `nextSibling`:返回与该节点同级的下一个节点，没有则返回null
    3. `lastChild`
4. 节点的类型,使用`nodeType`,可用来区分不同类型的节点，比如 元素, 文本 和 注释.参考:https://developer.mozilla.org/zh-CN/docs/Web/API/Node/nodeType
5. 节点的值`nodeValue`
...
![](../../picture/front/2-1-dynamicPageRendering.svg)
...
3. 注册(有以下三种方法).给事件目标**注册事件监听器**:
    1. `addEventListener(eventType,func,boolean/options)`:如`myButton.addEventListener('click', function(){alert('Hello world');}, false);`.然后可以通过`removeEventListener('click', func)`移除监听器,这两个方法在大型或者复杂的项目中比较有用.注意ie6-8不支持该方法.
        1. `boolean(useCapture)`参数:可选,默认为false.该参数是旧版DOM规范中规定的,用来标识该事件监听函数用于处理什么阶段的事件,只对捕获和冒泡阶段有用，对目标阶段无效,为true时表示用于处理冒泡阶段的事件,为false时表示用于处理捕获阶段的事件
        2. `options(object)`参数:可选,新版DOM规范中用来代替`userCapture`的.
        3. 该方法可以给同一个监听器注册多个处理器,而且不会互相覆盖.如,
...
4. 事件派发.当事件发生时， event 对象就会被创建并依次传递给事件监听器。处理函数中，将event对象作为第一个参数,可以访问event对象的内容.事件派发有3个阶段:捕获(capture phase),目标(target phase),冒泡(bubble phase).
    1. 事件派发的常见方法:
...
注意:当事件处理器被调用，事件处理器内部的 this 关键字被设置为注册该事件处理器的 DOM 元素。
