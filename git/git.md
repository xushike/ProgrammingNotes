...
[TOC]
# 一 概述
版本管理中主要包含**集中化的版本控制系统 (Centralized Version Control Systems，简称 CVCS)** 例如 CVS、Subversion 以及 Perforce 等 ，和**分布式版本控制系统(Distributed Version Control System，简称 DVCS)**, 例如Git、Mercurial、Bazaar 以及 Darcs  。DVCS的特点为每一次的克隆操作，实际上都是一次对代码仓库的完整备份。 其中文件的状态主要有4个：未记录(untracked)、已提交(committed)、已修改(modified)和已暂存(staged) 。

git是开源免费的分布式版本控制系统。除了版本,最核心的操作应该都是围绕分支进行的.官方的描述是At its core, the Git version control system is a content addressable filesystem.(Git是一套内容寻址文件系)
...
用于存储git的凭证(认证信息--账号密码等),多种helper可以同时存在

Git有以下几种helper:
1. `git-credential-cache`(`Cache`)：将凭据在内存中进行短时间的缓存,不会持久化到磁盘上，15分钟自动删除。
...
2. `git-credential-store`(`Store`)：将凭据明文保存在磁盘上,永不过期。但是明文存储不安全.
    ...
    2. 指定明文密码保存位置:可以为不同项目指定不同的文件从而达到使用多账号的目的.
        1. `git config --global credential.helper store --file=~/cred.txt`,但是该命令似乎并不生效,可以通过修改config文件达到目的,
            ...
            ```
3. `git-credential-manager`(`Manager`):安装了GitGUI，自动会在system级别中设置credential.helper为manager
    1. 老版本git是`manager`,新版本git是`manger-core`
4. `wincred`
