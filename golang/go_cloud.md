...
## 哈希（待整理）
哈希（Hash），又称散列，是一种将任意长度的输入数据通过特定算法映射为固定长度、近似唯一的输出数据的技术。核心是“数据指纹”和高效查找。这个特定算法称为哈希算法、散列算法或摘要算法，近似唯一也称为希望唯一(hopefully unique)，输出数据称为哈希值、散列值或摘要。
1. 特性：基础特性和密码学特性
    1. 基础特性
        1. 单向：无法从哈希值反推原始数据。也称不可逆、抗原性。
        2. 定长：输入是任意的，但输出是固定长度的
            1. 方便比较和查找
            2. 方便存储
            3. 方便给哈希函数定级
        3. 确定：同一输入永远得同一哈希值
        4. 雪崩：不同输入得到相同输出的概率极低。输入修改1bit，输出完全不同
    2. 密码学特性：共三个。三者的强度层级递增，满足强抗碰撞则另外两个安全特性自动满足，反过来则不成立。
        1. 抗原像(Preimage Resistance)：给定哈希值，无法找到任意原文。也称原像抗性。
        2. 抗第二原像(Second Preimage Resistance)：给定原文，找不到另一个不同原文。也称弱抗碰撞。
        3. 抗碰撞(Collision Resistance)：无法找到任意一对不同原文，使哈希值相等。也称强抗碰撞
2. 哈希碰撞(Hash Collision)：当两个不同的输入通过哈希函数得到了相同的输出时，就发生了哈希碰撞。也称为哈希冲突。
2. 常见应用场景
    1. 数据完整性验证：比如下载文件时，比较文件的MD5哈希值来验证文件的完整性
    2. 密码存储：不直接存储用户的密码，而是存储密码的哈希值
    3. 唯一性识别：不同的输入数据通常会生成不同的哈希值，这使得哈希值可以作为数据的唯一标识符。
    4. 数字签名

哈希的分类
1. 按安全等级
    1. 非密码学哈希（Non-Crypto Hash）：极致速度、低碰撞概率，不追求密码安全。常见的有MurmurHash、CityHash、HighwayHash、XXHash、Fnv
    2. 密码学安全哈希（Cryptographic Hash）
        1. MD5：输出128位(16字节)：速度快，但不安全。04年被攻破，仅用于非安全校验（如文件快速校验）
        2. SHA(Secure Hash Algorithm)系列
            1. SHA-0（已废弃）
            2. SHA-1：输出160位(20字节)，速度比MD5快，但也不够安全
            3. SHA‑2
                1. SHA‑256：密码学哈希函数，输出256位(32字节)，通常写成64个十六进制字符，安全性更高。
            4. SHA‑3
                1. 定长哈希
                    1. SHA-3‑256
                1. SHAKE: SHA-3的XOF(eXtendable-Output Function，可扩展输出函数)模式，变长，可输出任意长度
                    1. SHAKE128
                    2. SHAKE256
        3. SM3(国密)
        4. BLAKE
            ...
            2. BLAKE3
1. 按计算模式
    1. 全量哈希：一次计算全部全部输入数据。也可称为一次性哈希、整块哈希。缺点是大文件、网络流无法处理，内存占用高
    2. 流式哈希(Streaming Hash)：核心是维护一个中间状态（Context），数据可分批输入、随时追加，最终一次性输出结果。也称为增量哈希。所有现代密码哈希 (比如SHA256、BLAKE2) 都原生支持流式。
        1. 特性 
            1. 内存高效：仅维护固定大小上下文（如 SHA-256 为 64 字节），与输入数据总量无关
    3. 滚动哈希(Rolling Hash)：也称为滑动窗口哈希、特殊流式
2. 按输出特性
    1. 定长
    2. 变长
    3. 伸缩：支持长输出、并行。也称为海绵结构(Sponge Construction)哈希。
3. 写入顺序：顺序哈希 / 无序聚合哈希
加速特性：串行 / 并行 SIMD 哈希
1. 按计算效率分：快慢哈希
    1. 慢哈希：BCrypt（兼容广）、Argon2、PBKDF2、Scrypt


哈希和加密的区别(HASHING VS ENCRYPTION):哈希是摘要而不是加密，哈希也是一种单向函数。加密将明文数据转换为密文，然后在提供正确密钥后将其转换回明文，这是一种双向函数。与哈希不同，哈希不能逆转。 这是一个重要的区别。

### 慢哈希
字典攻击、暴力攻击、查询表、彩虹表和长度延展攻击
1. 彩虹表：彩虹表就是预先计算好的常用输入和MD5输出的对照表。以用户密码为例，暴力穷举会消耗大量的算力和时间，但用户使用了常见的密码，通过彩虹表一下就能反查到原始密码。防御彩虹表的方法是加盐(salt)`digest = md5(salt+inputPassword)`

为什么需要慢哈希：盐(salt)使攻击者无法采用特定的查询表和彩虹表快速破解大量哈希值，但是却不能阻止他们使用字典攻击或暴力攻击。高端的显卡（GPU）和定制的硬件可以每秒进行数十亿次哈希计算，因此这类攻击依然可以很高效。为了降低攻击者的效率，我们可以使用一种叫做密钥扩展的技术。这种技术的思想就是把哈希函数变得很慢，于是即使有着超高性能的GPU或定制硬件，字典攻击和暴力攻击也会慢得让攻击者无法接受。最终的目标是把哈希函数的速度降到足以让攻击者望而却步，但造成的延迟又不至于引起用户的注意。
