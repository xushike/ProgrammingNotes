...
2. `short`16位，2个字节
3. `int`和`Integer`:32位，4个字节。和go语言的int会随系统自动变成32位或64位不同，java的int不会随系统改变，是固定的32位4字节
4. `long`:64位
...
位运算/移位运算:计算机指令中的左移就是补0，但是右移分为两种：
1. 算术右移：移动后补的是最高位的值
...
## 8 泛型
为什么需要泛型：jdk1.5增加泛型支持主要是为了让集合能记住其元素的数据类型，次要目的是用于增强枚举类、反射等
1. 程序更健壮：可以在编译时检查集合中元素的类型，只要编译时期没有警告，那么运行时期就不会出现ClassCastException异常。
2. 代码更简洁：取出来使用的时候就不需要强制类型转换了
3. 可读性：编写集合的时候就限定了元素类型
4. 增强代码表达力：有些场景使用泛型能够减少重复代码逻辑，提升效率
...
### 1.11 执行exe文件报错: exe4j_java_home environment variable does not point to a working 64-bit JDK or JRE
可能原因
1. exe4j_java_home环境变量指向的目标不是JDK或JRE目录
2. exe4j_java_home环境变量指向的目录里的JDK或JRE不是64位

### 1.12 该版本的 %1 与你运行的 Windows 版本不兼容。请查看计算机的系统信息，然后联系软件发布者
入口的package不是main
