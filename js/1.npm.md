...
- devDependencies:  只有在开发应用时才会用到,指定项目开发所需要的模块,你不必在生产环境的应用中部署它们，当然，就算部署了也没什么坏处。以Angular举例:
    ...
    - `@angular/language-service`:Angular 的语言服务会分析组件模板，并且提供类型信息和错误信息，那些支持 TypeScript 的编辑机器可以使用它们来提升开发体验。比如vscode的Angular Language Service插件,可以提示`*ngIf`,`(click)`,`{{}}`等语法以及简单的语法错误检测.
