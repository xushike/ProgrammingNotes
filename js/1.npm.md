...
7. 参数`--save-dev`(老的写法是`-D`):安装模块并写入到当前项目`package.json`文件的`ddevDependencies`中
8. `--verbose`(待整理)
...
### 5.2 文件字段说明
#### scripts
指定运行脚本命令的npm命令行缩写,比如比如`start`指定了运行`npm run start`时，所要执行的命令,如
```json
"scripts": {
