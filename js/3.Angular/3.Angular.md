...
### 2.5 angular 6.x
参考:https://www.cnblogs.com/haogj/p/8988948.html
...
表达式上下文:典型的表达式上下文是这个组件实例,但也可包含组件之外的对象,比如模板输入变量 (`let hero`)和模板引用变量(`#heroInput`),如,
...
关于`ngOninit()`钩子和`constructor`构造函数
...
最佳实践:构造函数用于依赖注入或执行一些简单的初始化操作,`ngOnInit()`钩子主要用于执行组件的其它初始化操作或获取组件输入的属性值。
...
### 1.13 angular报错:Metadata version mismatch for module, found version 4, expected 3 ...
