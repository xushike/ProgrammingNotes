...
### 1.1 make命令
使用：
1. 不带target`make`：如果Make命令运行时没有指定target，默认会执行Makefile文件的第一个没有通配符的目标。
    
    ```Makefile
    # 执行make的时候执行的是test，因为test上面的目标有通配符
    %.o:%.c
        gcc -o $@ $<
...
这里的`.PHONY`是special built-in target names之一，其他的参考：http://www.gnu.org/software/make/manual/html_node/Special-Targets.html#Special-Targets
