...
## 0 构架和常见名词
### 一切皆文件、文件系统、进程
一切皆文件，Linux 中，几乎所有的系统资源都被表示为文件，包括：
1. 普通文件：文本文件、二进制文件等。
2. 目录：用于组织和访问文件系统的树状结构。
3. 设备文件：代表硬件设备（如硬盘、打印机等）。在`/dev`目录下，你可以找到各种设备文件。
4. 管道：用于进程间通信的一种特殊类型的文件。
5. 套接字：用于网络通信的文件。
6. 符号链接：指向另一个文件或目录的特殊类型的文件。

Linux的文件系统：Linux 使用虚拟文件系统（Virtual File Systems, VFS）来管理所有不同类型的文件，VFS 是一个抽象层，它为不同类型的文件系统（如ext4、XFS、Btrfs、NTFS等）提供了一个统一的接口，这样在使用时可以不用操心底层的存储介质和文件系统类型。

1. 参考
    1. https://zhuanlan.zhihu.com/p/650866028
2. VFS的4个主要对象
    1. 超级块（Superblock） ：超级块是对一个文件系统的描述，存放系统中已安装文件系统的有关信息。
    2. 文件索引节点（inode） ：索引节点是对一个 文件物理属性 的描述，存放关于具体文件的一般信息。我们的磁盘在进行分区、格式化的时候会分为两个区域，一个是 数据区，用于存储文件中的数据；另一个是inode区，用于存放inode table（inode表），inode table 中存放的是一个一个的inode（也称为inode节点），不同的inode对应不同的文件。
        1. 在同一个文件系统中，每个索引节点号都是唯一的
        2. 文件被删除时，inode仍然存在，直到该inode所占用的空间被其他文件覆盖或系统清理。
    3. 目录项对象（dentry） ：目录项是对一个 文件逻辑属性 的描述，存放目录项与对应文件进行链接的信息。
        1. 路径中的每一个部分被称作目录项，例如`/Users/my_name/book1.md`中，根目录是`/`，而`Users`，`my_name`和文件 `book1.md`都是目录项。
    4. 文件对象（file） ：存放打开文件与进程之间进行交互的有关信息。
3. linux的文件系统树:linux以分层目录来组织所有文件，第一级目录称为根目录(唯一)，对于连接到计算机的磁盘或存储设备，则是挂载到目录树的某个节点上，区别于win中的每个存储设备都有独自的文件系统
2. 使用
    1. 文件系统的安装和挂载

进程
1. 进程和`/proc`文件系统：在 Linux 中，进程是通过`/proc`文件系统进行管理的。`/proc`是一个特殊的文件系统，它是一个伪文件系统，它提供了一个接口，用于访问内核和进程信息。例如：
    1. `/proc/cpuinfo`：提供 CPU 信息的文件。
    2. `/proc/meminfo`：提供内存信息的文件。
    3. `/proc/[pid]`：每个运行中的进程都有一个以其 PID（进程 ID）命名的目录，其中包含该进程的详细信息，如环境变量、打开的文件等。
2. 进程的数据结构、标准输入输出、重定向和管道
    ...
    struct task_struct {
        ...
        // 存放文件系统信息的指针
        struct fs_struct      *fs;
        // files指针指向一个数组，包含该进程打开的文件指针，我们常说的「文件描述符」就是指这个files指针数组的索引
        // 一般来说，一个进程会从files[0]读取输入，将输出写入files[1]，将错误信息写入files[2]。每个进程被创建时，files的前三位被填入默认值，分别指向标准输入流、标准输出流、标准错误流。
    ...
    1. 标准输入输出
        1. 标准输入输出文件
            - 标准输入文件`stdin`：`files[0]`指向标准输入文件，进程从中得到输入，默认是键盘
            - 标准输出文件`stdout`：`files[1]`指向标准输出文件，进程运行结果输出到此，默认是屏幕
            - 标准错误输出文件`stderr`：`files[2]`指向标准错误输出文件，进程将错误信息(状态信息)输出到此,默认也是屏幕
        2. 标准输入输出的几个问题、重定向和管道出现的原因
            1. 问题
                - 每次的输入只能用一次
                - 修改不方便
                - 只能看不能动
            2. 于是linux引入了两种机制:输入/输出重定向和管道
    1. 通过进程的数据结构中的`files`指针来理解重定向和管道
        1. 重定向：对于一般的计算机，标准输入流是键盘，标准输出流是显示器，标准错误流也是显示器。如果我们写的程序需要其他资源，比如打开一个文件进行读写，就需要进行系统调用，让内核把文件打开，这个文件就会被放到files的第4个位置，即`files[3]`。这样输入重定向就很好理解了，程序想读取数据的时候会去`files[0]`读取，我们只要把`files[0]`指向一个文件，那么程序就会从这个文件中读取数据，而不是从键盘；输出重定向也是同理，把`files[1]`指向一个文件，那么程序的输出就不会写入到显示器，而是写入到这个文件中；错误重定向也是一样。
            
            ```bash
            pbcopy < myFile.txt
            ```
        2. 管道：和重定向异曲同工，把一个进程的输出流和另一个进程的输入流连接起来，就形成了一条"管道"，数据在其中传递
            
            ```bash
            cat myFile.txt | pbcopy
            ```
    2. 重定向：即I/O重定向，重定向允许我们更改输出位置和输入来源
        1. 标准输入重定向:把标准输入重定向到指定的文件中,即可以不来自键盘
            1. 直接跟文件名,如`wc hello.txt`
            2. 输入重定向:`[命令] < [文件名]`
            3. 将多行文本或代码块传递给交互式命令的Here-document(Heredoc) ：也被称为here-document/here-text/heredoc/hereis/here-string/here-script,允许命令在获取命令行输入数据时像是在读取文档一样,形如`命令 << 分隔串`，分割串代表开始和结束,如:
        ...
        5. 重定向到黑洞`/dev/null`：`/dev/null`是一个特殊文件,叫位存储桶,位存储桶是个古老的Unix概念.
    3. 管道：管道用于处理标准输出的数据。对于把**一个命令的输出作为另一个命令的输入**, 可以通过`|`一直连下去,最后显示最后一个的输出,如`ls ~ | wc -w`
        1. 说明
        ...
        2. 例子
            ...
            ```bash
            # 很多工具的安装脚本写在网页上，可以借助curl和sh来实现安装
            curl https://getcroc.schollz.com | bash
            ```

    4. 命令替换：命令替换和重定向有点类似,区别在于命令替换是把一个命令的输出作为另一个命令的参数,有两种用法,具体参考字符展开中的命令替换
    5. 连续命令：和管道不同在于,连续命令(用`;`分隔)中各个命令不存在相关性,只是顺序执行

### 环境变量
常见环境变量
1. `PATH`: 用于指定系统在执行命令时搜索可执行程序的路径

环境变量操作的相关工具
1. `env`:用于查看和设置环境变量
    1. 参数
        1. `-i(--ignore-environment)`:以空的环境执行命令，即不继承当前环境变量。
        2. `-u <ENV_NAME>`(`--unset=ENV_NAME`)，在执行命令时从环境中忽略指定的变量ENV_NAME。
    2. 使用
        ```bash
        # 不带参数使用是列出当前环境中的所有变量
        env
        ...
        env -i bash
        # 在执行cmd_name时，ENV_NAME变量不会被传递给该命令。
        env -u ENV_NAME cmd_name
        ```

环境变量的操作
1. 查看:
    1. `echo`: `echo`可以将环境变量输出到屏幕，比如`echo $PATH`
1. 修改
    1. 临时修改
    2. 持久化修改：通过
1. `export`可新增，修改或删除环境变量(注意没有查看环境变量)，供后续执行的程序使用。`export`的效力仅限于该次登陆操作。
...
1. 参数
    1. 参数`--full-time`：一般情况下似乎看不到年份信息，此命令可以显示全部日期信息
    2. 参数`-a`:列出所有文件,包括隐藏文件和以.开头的文件（linux中`.`代表当前目录,`..`代表父目录）
    3. 参数`-l`:以长格式(每个一行?)查看文件详细信息
    4. 参数`-t`按文件修改时间的先后来排序
    5. 参数`-h`(`--human-readable`),以人可读的格式来显示文件大小
    6. 参数`-i`(`--inode`):打印文件的索引节点(inode)信息
    7. 参数`-S`,命令输出结果按照文件大小来排序,优先级比`-t`高
    8. 参数`-r`(`--reverse`),以相反顺序来显示结果
    9.  参数`-A`,列出除`.`和`..`外所有文件,包括隐藏文件
2. 使用
...
会打印文件类型和内容的简单描述,包括编码信息等，比如文本文件是`ASCII text`，类型有：
|属性|	文件类型|
|-|:-|
|`-`|	一个普通文件|
|`d`|	一个目录|
...
`less`的设置
1. 设置查看文件时的编码格式：`export LESSCHARSET=utf-8`
...
### 2.2 文件系统跳转
1. 查看当前目录：`pwd`(`print working directory`),该命令会在结尾自动加上一个换行符
2. 更改工作目录,其中`.`是当前目录,`..`是父级目录,而且几乎所有情况下都可以省略`./`,因为它是隐含的。跳转时路径中间必须都是目录,不能包含文件,比如`cd ./xxx.txt`就不行
...
1. 变量相关
    1. 查看所有默认环境变量`env`
    2. 查看所有变量（包括环境变量、shell变量、本地变量）`set`
1. 别名`alias`
...
特殊字符说明：
1. 星号(`*`)：表示 cron 表达式能匹配该字段的所有值。如在第5个字段(month)使用星号，表示每个月
2. 斜线(`/`)：表示增长间隔，如第2个字段(minutes) 值是 3-59/15，表示每小时的第3分钟开始执行一次，之后每隔 15 分钟执行一次（即 3、18、33、48 这些时间点执行），这里也可以表示为：3/15
3. 逗号(`,`)：用于枚举值，如第6个字段值是 MON,WED,FRI，表示 星期一、三、五 执行
4. 连字号(`-`)：表示一个范围，如第3个字段的值为 9-17 表示 9am 到 5pm 直接每个小时（包括9和17）
5. 问号(`?`)：只用于日(Day of month)和星期(Day of week)，表示不指定值，同`*`
