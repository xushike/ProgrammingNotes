...
2. 查看进程`ps`
    ...
    1. 直接使用`ps`,只列出与当前终端会话相关的进程,其中TTY是“Teletype” 的简写，是指进程的控制终端。这里，Unix 展示它的年龄(?).TIME 字段表示 进程所消耗的 CPU 时间数量。
        ...
        ```
    2. 带选项`x`的用法`ps x`(注意不是`-x`),展示所有进程，不管它们由什么终端（如果有的话）控制。TTY栏的问号表示没有控制终端。该用法可以看到我们所拥有的每个进程的信息。
