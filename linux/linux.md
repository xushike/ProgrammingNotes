...
1. 查看文件信息
    1. `ls`:最基础并且使用的最广泛的命令行中工具之一。它是一个POSIX兼容工具，在GNU基本工具集以及BSD各种变体上都可以使用
        ...
        10. 参数`-A`,列出除`.`和`..`外所有文件,包括隐藏文件
2. 查看文件类型:`file [filename]`
    会打印文件类型和内容的简单描述,比如文本文件是`ASCII text`
3. 查看文本文件内容:`less`
    可查看所有类型的文本文件(即ASCII类型的文件,包括txt、配置文件、脚本等),如果查看非文本文件,则是乱码.特点是自带分页,可以接受标准输入.
    1. 查看时的键盘命令:上一页`b`,下一页`space`,页首`g`,页尾`G`,查找匹配`/xxx`,查看下一个匹配`n`,查看帮助`h`,退出less`q`
    2. less是早期unix程序more的改进版,意味着less is more
    3. 因为有分页,所以可以用来方便的查看产生标准输出的命令运行结果,比如`ls | less`
4. 查看命令类型:`type`会显示命令的类型:
    1. file为外部指令
    2. alias为别名
    3. builtin为系统內建指令
5. 查看可执行程序位置`which`
    有时系统中不止安装了程序的一个版本,虽然在桌面系统中不常见,但在大型系统中却很平常.
    1. 该命令对只对可执行程序有效，不包括内建命令和命令别名,有点奇特,比如对`cd`就无效
6. 查看內建命令的帮助文档`help`
    ...
    1. 许多可执行程序支持一个`--help`选项，用于显示命令所支持的语法和选项说明.不过有些不支持,但是任何情况该命令都值得一试
7. 查看程序手册`man`
...
4. 打印匹配行`grep`
    很强大,用来找到文件中的匹配文本,基本用法`grep pattern [file...]`,支持正则表达式.比如想找目录中文件名包含`xiaoke`的文件,可以这样`ls | grep xiaoke`
    1. 参数`-i`:忽略大小写
    2. 参数`-v`:只显示不匹配的行
...
4. 花括号展开
    从一个包含花括号的模式中创建多个文本字符串,常见的应用场景是:创建一系列的文件或目录列表.比如摄影师给大量的相片命名.
#### 2.5 其他
1. 别名`alias`
