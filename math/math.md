...
1. 幂(也叫power)和指数：幂是指乘方运算的结果。n^m指该式意义为m个n相乘。把n^m看作乘方的结果，叫做n的m次幂，也叫n的m次方，其中，n称为底数(base number)，m称为指数，写在上标，当不能用上标时，比如在编程语言或电子邮件中，通常写成n^m。幂是开方的逆运算。
	1. 指数（exponent）
		1. 负指数：非零数的负指数等于该数正指数的倒数
		2. 小数指数：先按分母指数开方，然后按分子指数乘方。更详细的略
