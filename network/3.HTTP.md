...
## 3 常识
### 3.1 HTTP的五个特点
1. 简单快速：请求只需方法、URL、协议版本和头部，协议本身开销小。
2. 无连接：每个请求/响应完成后，服务器不保留该次请求的连接状态（应用层上不依赖连接）；这意味着请求结束后可以关闭连接。
3. 无状态：务器不记住前一次请求的信息，每次请求都是独立的；如果要维持会话，需要额外机制（比如Cookie/Session）。
4. 媒体独立：任何类型的数据都可以通过HTTP传输，靠`Content-Type`指明格式。
5. 支持C/S和B/S模式：既适合传统客户端-服务器应用，也适合浏览器-服务器模式
